...
class Classifier():
    ...
    def defind_model(self):
        ...
        self.model = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(len(self.label_names),
                                  activation='softmax', dtype=tf.float32)
        ])
        print("summary 打印开始")
        # self.model.summary()
        # raise Exception('打印')
    ...
    def reorg_train_valid(data_dir, train_dir, input_dir, valid_ratio, idx_label):
        ...
        # data_dir,train_dir  = '../../data/kaggle_dog' ;'train'
        for train_file in os.listdir(os.path.join(data_dir, train_dir)):
            ...
            if label not in label_count or label_count[label] < n_valid_per_label:
                ...
                label_count[label] = label_count.get(
                    label, 0) + 1  # lable_count 用于计数用
            else:
    ...
    def reorg_dog_data(self, data_dir, label_file, train_dir,
        ...
        with open(os.path.join(data_dir, label_file), 'r') as f:
            ...
            self.idx_label = dict(((idx, label)
                                  for idx, label in tokens))  # 通过标签数据文件读取标签字典
        # 整理验证集和训练集文件
    ...
    def load_data_lable(self):
        ...
        label_to_index = dict((name, index) for index, name in enumerate(
            self.label_names))  # lable to index relationship

        # 记录图片地址
        train_all_image_paths = [str(path)
                                 for path in list(train_data_root.glob('*/*'))]
        valid_all_image_paths = [str(path)
                                 for path in list(valid_data_root.glob('*/*'))]
        train_valid_all_image_paths = [
            str(path) for path in list(train_valid_data_root.glob('*/*'))]
        test_all_image_paths = [str(path)
                                for path in list(test_data_root.glob('*/*'))]

        # 记录图片目录对应的标签索引 疑问: 什么作用
        train_all_image_labels = [label_to_index[pathlib.Path(
            path).parent.name] for path in train_all_image_paths]
        valid_all_image_labels = [label_to_index[pathlib.Path(
            path).parent.name] for path in valid_all_image_paths]
        train_valid_all_image_labels = [label_to_index[pathlib.Path(
            path).parent.name] for path in train_valid_all_image_paths]
        #
        # 疑问: -1?
    ...
    def defind_train_func(self):
        ...
        self.callback = tf.keras.callbacks.LearningRateScheduler(
            scheduler)  # model.fit() 的回调函数
    ...
    def train_model(self):
        """# #### 9.13.6. 训练模型"""
        self.model.fit(self.train_ds, epochs=1,
                       validation_data=self.valid_ds,  callbacks=[self.callback])
        ...
        self.model.compile(optimizer=keras.optimizers.SGD(learning_rate=self.lr, momentum=0.9),
                           loss='sparse_categorical_crossentropy')
        self.model.fit(self.train_valid_ds, epochs=1,
    ...
    def write_ret(self, predictions):
        ...
        ids = sorted(os.listdir(os.path.join(
            self.data_dir, self.input_dir, 'test/unknown')))
